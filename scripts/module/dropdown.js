 ...
 */
 ...
 export function displayFilterMenu(displayMediaList) {
    ...
    //sélection du premier enfant de l'élément filter select
    const firstFilterOption = document.querySelector(
      ".filter-select a:first-child"
    );
    //sélection du dernier enfant de l'element filter select
    ...
    for (const filter of filterOptions) {
      filter.addEventListener("click", function (e) {
        e.preventDefault();
        // si un filtre ne contient pas la classe selected alors sélectionne le premier parent du filtre qui contient la classe
        ...
        if (!this.classList.contains("selected")) {
          ...
          // et remplace le texte (passe le filtre sélectionné en haut de liste)
